...
namespace TaskSchedulerManager.Core
{
    ...
    {
        private const string TaskFolder = "\\MyAppLaunchers\\";

        public static bool RegisterTasks(SchedulerProfile profile, out string message)
        {
            try
            {
                using (TaskService ts = new TaskService())
                {
                    ...
                    if (folder == null)
                        folder = ts.RootFolder.CreateFolder("MyAppLaunchers");

                    // 清理旧任务
                    var existingTasks = folder.GetTasks().Where(t => t.Name.StartsWith(profile.TaskNamePrefix));
                    ...
                    td.RegistrationInfo.Description = $"管理 {profile.Apps.Count} 个应用程序的启动和监控";
                    td.RegistrationInfo.Author = Environment.UserName;
                    td.Settings.StartWhenAvailable = true;
                    td.Settings.DisallowStartIfOnBatteries = false;
                    td.Settings.StopIfGoingOnBatteries = false;
                    td.Settings.AllowHardTerminate = true;
                    td.Settings.ExecutionTimeLimit = TimeSpan.Zero; // 无限制
                    td.Settings.Priority = System.Diagnostics.ProcessPriorityClass.Normal;

                    // 触发器：系统启动
                    BootTrigger bootTrigger = new BootTrigger();
                    bootTrigger.Delay = TimeSpan.FromSeconds(profile.BootDelaySeconds);
                    ...
                    // 操作：执行 PowerShell 监控模式
                    string powerShellPath = @"C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe";
                    string arguments = $"-ExecutionPolicy Bypass -NoProfile -File \"{scriptPath}\" -MonitorMode";

                    td.Actions.Add(new ExecAction(powerShellPath, arguments, scriptDir));

                    // 运行身份设置
                    if (profile.RunWithHighestPrivileges)
                    {
                        td.Principal.RunLevel = TaskRunLevel.Highest;
                    }

                    // 注册任务
                    var userId = profile.RunWhetherUserLoggedOn ? "SYSTEM" : WindowsIdentity.GetCurrent().Name;
                    folder.RegisterTaskDefinition(taskName, td,
                        TaskCreation.CreateOrUpdate,
                        userId,
                        null,
                        profile.RunWhetherUserLoggedOn ? TaskLogonType.ServiceAccount : TaskLogonType.InteractiveToken);

                    message = $"成功注册任务: {taskName}\n脚本位置: {scriptPath}\n日志位置: {logPath}";
                    return true;
                }
            }
            catch (Exception ex)
