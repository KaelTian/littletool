using NetworkFileTransfer.Upgrade;
using System.Net;

namespace NetworkFileTransfer
{
    public partial class FrmMainUpgrade : Form
    {
        private UpgradeFileTransferServer? _server;
        private UpgradeFileTransferClient? _client;
        ...
        {
            _server = new UpgradeFileTransferServer()
            {
        ...
        private void OnTransferStarted(object? sender, TransferEvent e)
        {
            if (InvokeRequired)
            {
                Invoke(new Action<object, TransferEvent>(OnTransferStarted), sender, e);
                return;
            }

            // 重置传输相关变量
            _totalBytes = e.FileSize;
            _transferredBytes = 0;
        ...
        // 这里保持和原方法相同的方法签名、跨线程处理逻辑
        private void OnTransferCompleted(object? sender, TransferEvent e)
        {
            // 与原方法保持一致：检查是否需要跨线程调用（WinForm 控件线程安全要求）
            if (InvokeRequired)
            {
                // 异步调用自身，确保在UI线程执行操作，避免跨线程访问控件异常
                Invoke(new Action<object?, TransferEvent>(OnTransferCompleted), sender, e);
                return;
            }
        ...
        private void OnClientConnected(object? sender, TransferEvent e)
        {
            if (InvokeRequired)
            {
                Invoke(new Action<object, TransferEvent>(OnClientConnected), sender, e);
                return;
            }
        ...
        {
            _client = new UpgradeFileTransferClient();
            _client.Connected += OnClientConnected;
            _client.ProgressChanged += OnProgressChanged;
            _client.TransferStarted += OnTransferStarted;
            _client.TransferCompleted += OnTransferCompleted;
        ...
        private void OnProgressChanged(object? sender, TransferProgressEvent e)
        {
            if (InvokeRequired)
            {
                Invoke(new Action<object, TransferProgressEvent>(OnProgressChanged), sender, e);
                return;
            }

            _transferredBytes = e.BytesTransferred;
            _totalBytes = e.FileSize;
