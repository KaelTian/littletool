...
namespace _005Tools
{
    ...
    public class VerticalGauge : UserControl
    {
        ...
        private int _decimalPlaces = -1;  // ── 改动：-1 表示自动推断

        private Color _scaleBackColor = Color.FromArgb(128, 0, 128);
        private Color _scaleTextColor = Color.White;
        private Color _warnZoneColor = Color.FromArgb(255, 235, 59);
        private Color _errorLineColor = Color.FromArgb(244, 67, 54);
        ...
        /// <summary>
        /// 小数点位数,-1 表示自动推断（根据 Min/Max/Warn/Error 的实际值来决定显示几位小数，整数则不显示小数点）
        /// </summary>
        // ── 改动：-1 为自动，0/1/2… 为强制指定 ──
        [Category("外观"), DefaultValue(-1), Description("小数位数，-1 表示自动推断")]
        ...
        // ── 新增：自动推断所有关键值的最大有效小数位 ──
        private int InferDecimalPlaces()
        {
            float mid = (_minValue + _maxValue) / 2f;
            var candidates = new[] { _minValue, _maxValue, _warnValue, _errorValue, mid };
            int maxDp = 0;

            foreach (var v in candidates)
            {
                // 用 G6 避免科学计数法，精度足够且稳定
                string s = v.ToString("G6", System.Globalization.CultureInfo.InvariantCulture);
                int dot = s.IndexOf('.');
                if (dot >= 0)
                {
                    // 去掉末尾无意义的 0，计算真实有效小数位
                    int len = s.Length;
                    while (len > dot + 1 && s[len - 1] == '0')
                        len--;
        ...
        private void DrawScale(Graphics g, RectangleF scaleRect, RectangleF gaugeRect)
        {
            ...
            // ── 改动：自动推断或取用户强制值 ──
            int effectiveDp = _decimalPlaces >= 0 ? _decimalPlaces : InferDecimalPlaces();
            string fmt = effectiveDp > 0 ? $"F{effectiveDp}" : "F0";
